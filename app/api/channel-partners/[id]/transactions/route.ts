...
export async function GET(
  ...
  try {
    ...
    console.error(`Error loading transactions for partner ${id}:`, error);
...
export async function POST(
  ...
  try {
    await dbConnect();
    // Only Admins, Super Admins, and Managers can record payouts
    ...
    // Verify this commission is indeed owned by the CP
    const commission = await Commission.findOne({ _id: data.commissionId, entityId: id, entityType: "ChannelPartner" });
    if (!commission) {
      return NextResponse.json({ error: "Commission record not found or does not belong to this partner" }, { status: 404 });
    }
    ...
    console.error(`Error recording transaction for partner ${id}:`, error);
